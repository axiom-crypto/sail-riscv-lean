...
inductive instruction where
  ...
  | OPENVM_TERMINATE (_ : (regidx × regidx × (BitVec 12)))
  | OPENVM_HINT_STOREW (_ : (regidx × regidx))
  | OPENVM_HINT_BUFFER (_ : (regidx × regidx))
  | OPENVM_REVEAL (_ : (regidx × regidx × (BitVec 12)))
  | OPENVM_PHANTOM (_ : (regidx × regidx × (BitVec 12)))
...
abbrev openvm_public_values_size : Int := 536870912



abbrev reveal_index := Int
...
structure frozen_program_entry where
  pc : xlenbits
  raw_inst : instbits
  deriving BEq, Inhabited, Repr
...
abbrev RegisterType : Register → Type
  ...
  | .frozen_program => (List frozen_program_entry)
  | .reveal => (Vector (BitVec 8) 536870912)
  | .hint_oracle => (List (BitVec 8))
  | .satp => (BitVec 32)
...
instance : Inhabited (RegisterRef RegisterType (List (BitVec 8))) where
  default := .Reg hint_oracle
instance : Inhabited (RegisterRef RegisterType (List frozen_program_entry)) where
  default := .Reg frozen_program
...
instance : Inhabited (RegisterRef RegisterType (Vector (BitVec 8) 536870912)) where
  default := .Reg reveal
