...
/-- Type quantifiers: k_ex475406_ : Bool -/
...
def hartSupports (merge_var : extension) : Bool :=
  ...
  | .Ext_U => false
  | .Ext_H => false
  | .Ext_Zibi => ((sys_enable_experimental_extensions ()) && (false : Bool))
  | .Ext_Zic64b => false
  | .Ext_Zicbom => false
  | .Ext_Zicbop => false
  | .Ext_Zicboz => false
  ...
  | .Ext_Zama16b => false
  | .Ext_Zawrs => false
  | .Ext_Za64rs => ((plat_reservation_set_size_exp ≤b 6) && ((false : Bool) || (false : Bool)))
  | .Ext_Za128rs => ((plat_reservation_set_size_exp ≤b 7) && ((false : Bool) || (false : Bool)))
  | .Ext_Zfa => false
  | .Ext_Zfbfmin => false
  | .Ext_Zfh => false
  | .Ext_Zfhmin => false
...
def currentlyEnabled (merge_var : extension) : SailM Bool := do
  ...
  | .Ext_Xopenvm => (pure (hartSupports Ext_Xopenvm))
  | .Ext_H => (pure false)
  | .Ext_Zicbom => (pure false)
  | .Ext_Zicboz => (pure false)
  | .Ext_Zkr => (pure false)
  | .Ext_Zicsr => (pure false)
  | .Ext_Zicntr => (pure false)
  | _ =>
    (do
      assert false "Pattern match failure at openvm/openvm_profile.sail:15.0-15.52"
      throw Error.Exit)
termination_by (let ext := merge_var
...
/-- Type quantifiers: k_ex476388_ : Bool, width : Nat, width ∈ {1, 2, 4, 8} -/
...
def encdec_forwards (arg_ : instruction) : SailM (BitVec 32) := do
  ...
  | .OPENVM_TERMINATE (rd, rs1, code) =>
    (do
      ...
      then
        (pure ((code : (BitVec 12)) +++ ((encdec_reg_forwards rs1) +++ (0b000#3 +++ ((encdec_reg_forwards
                    rd) +++ 0b0001011#7)))))
      else
        (do
          assert false "Pattern match failure at unknown location"
  ...
  | .OPENVM_HINT_STOREW (rd, rs1) =>
    (do
      if ((← (currentlyEnabled Ext_Xopenvm)) : Bool)
      then
        (pure (0b000000000000#12 +++ ((encdec_reg_forwards rs1) +++ (0b001#3 +++ ((encdec_reg_forwards
                    rd) +++ 0b0001011#7)))))
      else
        (do
          assert false "Pattern match failure at unknown location"
  ...
  | .OPENVM_HINT_BUFFER (rd, rs1) =>
    (do
      if ((← (currentlyEnabled Ext_Xopenvm)) : Bool)
      then
        (pure (0b000000000001#12 +++ ((encdec_reg_forwards rs1) +++ (0b001#3 +++ ((encdec_reg_forwards
                    rd) +++ 0b0001011#7)))))
      else
        (do
          assert false "Pattern match failure at unknown location"
  ...
  | .OPENVM_REVEAL (rd, rs1, imm) =>
    (do
      if ((← (currentlyEnabled Ext_Xopenvm)) : Bool)
      then
        (pure ((imm : (BitVec 12)) +++ ((encdec_reg_forwards rs1) +++ (0b010#3 +++ ((encdec_reg_forwards
                    rd) +++ 0b0001011#7)))))
      else
        (do
          assert false "Pattern match failure at unknown location"
  ...
  | .OPENVM_PHANTOM (rd, rs1, imm) =>
    (do
      if ((← (currentlyEnabled Ext_Xopenvm)) : Bool)
      then
        (pure ((imm : (BitVec 12)) +++ ((encdec_reg_forwards rs1) +++ (0b011#3 +++ ((encdec_reg_forwards
                    rd) +++ 0b0001011#7)))))
      else
        (do
          assert false "Pattern match failure at unknown location"
...
def plat_clint_base : physaddrbits := unwrapValue ((to_bits_checked (l := 34) (536870912 : Int)))

def plat_clint_size : physaddrbits := unwrapValue ((to_bits_checked (l := 34) (0 : Int)))

def plat_have_sig : Bool := false

def plat_sig_base : physaddrbits := unwrapValue ((to_bits_checked (l := 34) (0 : Int)))
