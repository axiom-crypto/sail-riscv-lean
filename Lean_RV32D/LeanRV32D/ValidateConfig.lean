...
def check_tvecs (_ : Unit) : Bool :=
  ...
  if (((false : Bool) && ((not (true : Bool)) && (not (true : Bool)))) : Bool)
  then
    (let valid : Bool := false
    let _ : Unit :=
      (print_endline
...
def check_mmu_config (_ : Unit) : SailM Bool := do
  ...
  let _ : Unit :=
    if (((not (false : Bool)) && (false : Bool)) : Bool)
    then
      (let valid : Bool := false
      (print_endline
...
def check_vext_config (_ : Unit) : Bool :=
  ...
  let valid : Bool :=
    if (((vector_support_ge vector_support_level Float_single) && (not (false : Bool))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
  ...
  let valid : Bool :=
    if (((hartSupports Ext_Zve32x) && (not (false : Bool))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
  ...
  let valid : Bool :=
    if (((false : Bool) && ((not (hartSupports Ext_Zve32f)) || (not (false : Bool)))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
...
/-- Type quantifiers: k_ex501859_ : Bool -/
    ...
    (do
      ...
      let clint_ok ← do
        (pure ((not clint_supported) || (← (within_configured_pma_memory "CLINT (platform.clint)"
                (some IOMemory) (← (to_bits_checked (l := 64) (536870912 : Int)))
                (← (to_bits_checked (l := 64) (0 : Int)))))))
      ...
      let sig_ok ← do
        (pure ((not sig_supported) || (← (within_configured_pma_memory
                ...
                (← (to_bits_checked (l := 64) (0 : Int))) (zero_extend (m := 64) plat_sig_size)))))
...
/-- Type quantifiers: k_ex502014_ : Bool -/
...
def check_misc_extension_dependencies (_ : Unit) : Bool :=
  ...
  let valid : Bool :=
    if (((false : Bool) && (not ((false : Bool) || (false : Bool)))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
  ...
  let valid : Bool :=
    if (((false : Bool) && (not
           ((false : Bool) && ((false : Bool) && ((false : Bool) || (false : Bool)))))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
  ...
  let valid : Bool :=
    if (((false : Bool) && ((not (false : Bool)) || (not (false : Bool)))) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
  ...
  let valid : Bool :=
    if (((false : Bool) && (((Sail.BitVec.extractLsb sys_writable_hpm_counters 31 3) &&& (Sail.BitVec.extractLsb
...
def check_extension_param_constraints (_ : Unit) : Bool :=
  let valid : Bool := true
  let valid : Bool :=
    if (((false : Bool) && (plat_cache_block_size_exp != 6)) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
  ...
  let valid : Bool :=
    if ((((false : Bool) || (false : Bool)) && (plat_reservation_set_size_exp <b min_rss_exp)) : Bool)
    then
      (let valid : Bool := false
      let _ : Unit :=
        (print_endline
    ...
    then
      (let valid : Bool :=
        if ((misaligned_exception_is_access_fault
             ({ load_store := (some AlignmentException)
                ...
                lrsc := AlignmentException } : GlobalMisalignedExceptions).load_store) : Bool)
        then
          (let valid : Bool := false
          let _ : Unit :=
            (print_endline
      ...
      if ((misaligned_exception_is_access_fault
           ({ load_store := (some AlignmentException)
              vector := (some AlignmentException)
              amo := (some AlignmentException)
              lrsc := AlignmentException } : GlobalMisalignedExceptions).vector) : Bool)
      then
        (let valid : Bool := false
        let _ : Unit :=
          (print_endline
...
def check_stateen_config (_ : Unit) : Bool :=
  if (((not (false : Bool)) && (not (false : Bool))) : Bool)
  then true
  else
    (let valid : Bool := true
    let valid : Bool :=
...
def check_physaddr_bits (_ : Unit) : Bool :=
  ...
  then
    ...
    then
      ...
      (if (((physaddr_bits_nat >b 32) && ((xlen == 32) && (not (false : Bool)))) : Bool)
      then
        (let _ : Unit :=
          (print_endline
            (HAppend.hAppend "`memory.physaddr_bits` is "
